...
func (r *ComponentRegistrarImpl) RegisterComponent(config *ComponentConfig) error {
	r.mutex.Lock()
...
func (r *ComponentRegistrarImpl) GetComponentByType(componentType ComponentType) ([]ComponentInterface, error) {
	r.mutex.RLock()
	defer r.mutex.RUnlock()

	components, exists := r.componentTypeIndex[componentType]
	if !exists {
		return nil, fmt.Errorf("components not found of type %d", componentType)
	}
