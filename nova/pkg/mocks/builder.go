...
func (m *MockPipelineBuilder) AddTask(task build.BuildTaskInterface) (build.PipelineBuilderInterface, error) {
	args := m.Called(task)
	return args.Get(0).(build.PipelineBuilderInterface), args.Error(1)
}

// Build is a mock implementation for constructing the pipeline.
func (m *MockPipelineBuilder) Build() (build.BuildPipelineInterface, error) {
	args := m.Called()
	return args.Get(0).(build.BuildPipelineInterface), args.Error(1)
}
