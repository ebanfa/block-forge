...
type BuildProjectOpFactory struct {
}

// CreateComponent creates a new instance of the BuildService.
func (bf *BuildProjectOpFactory) CreateComponent(config *configApi.ComponentConfig) (component.ComponentInterface, error) {
	// Construct the service
	return NewBuildProjectOp(config.ID, config.Name, config.Description), nil
}

// BaseComponent represents a concrete implementation of the SystemOperationInterface.
type BuildProjectOp struct {
	system.BaseSystemOperation // Embedding BaseComponent
}

// Type returns the type of the component.
func (bo *BuildProjectOp) Type() component.ComponentType {
	return component.BasicComponentType
}

func NewBuildProjectOp(id, name, description string) *BuildProjectOp {
	return &BuildProjectOp{
...
func (bo *BuildProjectOp) Execute(ctx *context.Context,
